...
function ProjectDetails() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // 1. Fetch Profile (to determine role/layout)
            const profileRes = await fetch(`${API_BASE_URL}/auth/me`, { headers: { 'Authorization': `Bearer ${token}` } });
            if (profileRes.ok) {
            ...
            const projectRes = await fetch(`${API_BASE_URL}/projects/${id}`, { headers: { 'Authorization': `Bearer ${token}` } });
            if (projectRes.ok) {
            ...
            // 3. Fetch Meetings & Actions
            const meetingsRes = await fetch(`${API_BASE_URL}/projects/${id}/meetings`, { headers: { 'Authorization': `Bearer ${token}` } });
            if (meetingsRes.ok) setMeetings(await meetingsRes.json());

            const actionsRes = await fetch(`${API_BASE_URL}/projects/${id}/action-items`, { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    // Helper to determine role for Layout
    const getRoleLabel = () => {
    ...
    // Helper to get Nav Items (Go Back)
    const getNavItems = () => {
        // We can just provide a "Back to Dashboard" item
        return [
    ...
    const handleTabChange = (tabId) => {
        if (tabId === 'dashboard') {
            ...
            else if (role === 'PROJECT_HEAD') navigate('/project-head');
    ...
    const handleCreateMeeting = async (e) => {
        ...
        try {
            const token = sessionStorage.getItem('access_token');
            const res = await fetch(`${API_BASE_URL}/projects/${id}/meetings`, {
                method: 'POST',
    ...
    const handleAddMoM = async (meetingId) => {
    ...
    const handleCreateAction = async (e) => {
        ...
        try {
            const token = sessionStorage.getItem('access_token');
            const res = await fetch(`${API_BASE_URL}/projects/${id}/action-items`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    const handleUpdateActionStatus = async (actionId, newStatus) => {
        try {
            const token = sessionStorage.getItem('access_token');
            const res = await fetch(`${API_BASE_URL}/action-items/${actionId}/status`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        ...
        <div style={{ display: 'grid', gap: '24px' }}>
            ...
            <div className="grid-auto-fit">
                ...
                <div className="table-card" style={{ padding: '20px' }}>
                    ...
                    <div style={{ fontSize: '1.2rem', fontWeight: '600', color: 'var(--error-red)' }}>{new Date(project.deadline).toLocaleDateString()}</div>
                </div>
                <div className="table-card" style={{ padding: '20px' }}>
                ...
                <div className="table-card">
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '8px' }}>
                            {project.required_skills ? project.required_skills.split(',').map((skill, i) => (
                                <span key={i} style={{
                                    padding: '4px 12px',
                            ...
                            {project.assignees_details.map(u => (
                                <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px', borderRadius: '8px', backgroundColor: '#f9f9f9' }}>
                                    ...
                                    <div style={{ overflow: 'hidden' }}>
                                        <div style={{ fontWeight: '600', color: 'var(--text-dark)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u.full_name}</div>
                                        <div style={{ fontSize: '0.8rem', color: 'var(--text-light)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u.email}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
    ...
    const renderMeetings = () => (
        <div className="table-card">
            <div className="list-header">
                ...
                {['FACULTY', 'ADMIN', 'PROJECT_HEAD'].includes(profile?.role_key) && (
                ...
                <div style={{ margin: '0 0 24px 0', padding: '24px', backgroundColor: '#f9fafb', borderRadius: '12px', border: '1px solid #eee' }}>
                    ...
                    <form onSubmit={handleCreateMeeting}>
                        ...
                        <div style={{ display: 'flex', gap: '12px' }}>
                            ...
                            <button type="button" onClick={() => setShowMeetingModal(false)} className="btn-secondary" style={{ padding: '10px 20px', border: '1px solid #ddd', background: 'white', borderRadius: '8px', cursor: 'pointer' }}>Cancel</button>
                        </div>
                    </form>
                </div>
            )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {meetings.map(m => (
                        <div key={m.id} style={{ border: '1px solid #eee', borderRadius: '12px', padding: '20px', backgroundColor: 'white' }}>
                            ...
                            <p style={{ margin: '0 0 16px 0', color: '#4b5563' }}><strong>Agenda:</strong> {m.agenda || 'N/A'}</p>
                            ...
                            <div style={{ padding: '16px', backgroundColor: '#f3f4f6', borderRadius: '8px', borderLeft: '4px solid var(--primary-green)' }}>
                                ...
                                <div style={{ whiteSpace: 'pre-wrap', color: '#374151' }}>{m.mom_content || <em style={{ color: '#999' }}>Not yet recorded.</em>}</div>
                            </div>

                            {['FACULTY', 'ADMIN', 'PROJECT_HEAD'].includes(profile?.role_key) && (
                                <button className="icon-btn" style={{ marginTop: '12px', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px' }} onClick={() => handleAddMoM(m.id)}>
                                    ✏️ {m.mom_content ? "Edit MoM" : "Add MoM"}
                                </button>
                            )}
                        </div>
                    ))}
                ...
                <div style={{ margin: '0 0 24px 0', padding: '24px', backgroundColor: '#f9fafb', borderRadius: '12px', border: '1px solid #eee' }}>
                    ...
                    <form onSubmit={handleCreateAction}>
                        ...
                        <div style={{ display: 'flex', gap: '12px' }}>
                            ...
                            <button type="button" onClick={() => setShowActionModal(false)} className="btn-secondary" style={{ padding: '10px 20px', border: '1px solid #ddd', background: 'white', borderRadius: '8px', cursor: 'pointer' }}>Cancel</button>
                        </div>
                    </form>
                </div>
            )}
                        ...
                        <tbody>
                            {actions.map(a => (
                                <tr key={a.id}>
                                    ...
                                    <td><span className="badge badge-gray">{a.assigned_to.substring(0, 8)}...</span></td>
                                    <td>
                                        <span className={`badge ${a.urgency === 'Critical' ? 'badge-danger' :
                                            a.urgency === 'High' ? 'badge-warning' :
                                                a.urgency === 'Medium' ? 'badge-gray' : 'badge-success'
                                            }`}>
                                            {a.urgency}
                                        </span>
                                    </td>

...
function StudentDashboard() {
    ...
    const fetchActions = async () => {
        try {
            const token = sessionStorage.getItem('access_token');
            const response = await fetch(`${API_BASE_URL}/action-items/me`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
    ...
    const handleJoinOrg = async (e) => {
        ...
        try {
            const token = sessionStorage.getItem('access_token');
            const response = await fetch(`${API_BASE_URL}/auth/me`, { // Using generic update for join
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                alert('Organization joined successfully!');
                fetchProfile();
            } else {
                const data = await response.json();
                alert(data.detail || 'Failed to join organization');
            }
        } catch (err) {
            alert('Network error');
        } finally {
    ...
    // Auto-init edit form data when profile loads if missing org
    useEffect(() => {
                    ...
                    <div className="table-card">
                        <div className="list-header">
                            <div>
                                ...
                                <p style={{ color: 'var(--text-light)', fontSize: '0.9rem', marginTop: '4px' }}>Projects you are currently working on</p>
                            </div>
                        </div>
                        {projects.length === 0 ? (
                                ...
                                <table className="data-table">
                                    ...
                                    <tbody>
                                        {projects.map(p => (
                                            <tr key={p.id}>
                                                <td>
                                                    <Link to={`/projects/${p.id}`} style={{ fontWeight: '600', color: 'var(--primary-green)', textDecoration: 'none' }}>
                                                        {p.title}
                                                    </Link>
                                                    <div style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: '2px', maxWidth: '300px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.description}</div>
                                                </td>
                                                <td>
                                                    <span className={`badge ${p.status === 'Completed' ? 'badge-success' : p.status === 'In Progress' ? 'badge-warning' : 'badge-gray'}`}>
                                                        {p.status}
                                                    </span>
                                                </td>
                                                ...
                                                <td style={{ textAlign: 'right' }}>
                                                    <Link to={`/projects/${p.id}`} className="icon-btn">
                                                        👁️
                                                    </Link>
                                                </td>
                                            </tr>
                                    ...
                                    <tbody>
                                        {actions.map(a => (
                                            <tr key={a.id}>
                                                ...
                                                <td>
                                                    <span className={`badge ${a.urgency === 'Critical' ? 'badge-error' :
                                                ...
                                                <td>{a.status}</td>
                                                <td style={{ textAlign: 'right' }}>
                                                    <Link to={`/projects/${a.project_id}`} className="icon-btn">
                                                        👁️ View Project
                                                    </Link>
                                                </td>
                                            </tr>
                    ...
                    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
                        ...
                        <div className="table-card" style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '24px' }}>
                            <div style={{
                                ...
                                backgroundColor: 'var(--secondary-blue)', color: 'white',
                            ...
                            <div>
                                ...
                                <span className="badge badge-gray" style={{ fontSize: '0.9rem' }}>Student</span>
                            </div>
                        </div>

  ...
  return (
    <div className="qb-layout">
      ...
      <section className="qb-editor">
        ...
        {saveError && <p className="ad-error qb-save-error">{saveError}</p>}
        ...
        <AnimatePresence mode="wait">
          {selected ? (
            ...
            >
              ...
              <div className="qb-image-block">
                <div className="qb-row">
                  <label className="qb-grow">
                    Image (optional)
                    <input
                      value={selected.image ?? ""}
                      onChange={(e) => patchQuestion(selected.id, { image: e.target.value || undefined })}
                      placeholder="/survey/1.jpg or https://\u2026"
                    />
                  </label>
                </div>
                <div className="qb-image-tools">
                  <label className="qb-ghost qb-upload">
                    Upload image
                    <input
                      type="file"
                      accept="image/*"
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          patchQuestion(selected.id, { image: dataUrl });
                        } catch {
                  ...
                  {selected.image && (
                    <button className="qb-danger" onClick={() => patchQuestion(selected.id, { image: undefined })}>
                      Remove image
                    </button>
                  )}
                </div>
                {imageError && <p className="ad-error qb-save-error">{imageError}</p>}
                {selected.image && (
                  <div className="qb-image-preview">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={selected.image} alt="Question preview" />
                  </div>
                )}
              </div>
...
async function fileToResizedDataUrl(file: File, maxSize: number, quality: number): Promise<string> {

...
export default function OrgAdminList({ org, onBack, onCreateCallback }: Props) {
  ...
  const fetchAdmins = async () => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/organizations/${org.id}/users`, { headers: authHeaders() });
  ...
  const handleDelete = async (userId: string) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE_URL}/organizations/${org.id}/admins/${userId}`, {
        method: "DELETE",
        headers: authHeaders(),
      });

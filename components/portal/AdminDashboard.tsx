...
export default function AdminDashboard() {
  ...
  const fetchPendingApprovals = async () => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/admin/approvals`, { headers: authHeaders() });
  ...
  const handleApprove = async (userId: string) => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/admin/approve/${userId}`, {
        method: "POST",
        headers: authHeaders(),
      });
      if (response.ok) {
  ...
  const handleReject = async (userId: string) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE_URL}/admin/reject/${userId}`, {
        method: "POST",
        headers: authHeaders(),
      });
      if (response.ok) {
  ...
  const fetchActionItems = async () => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/action-items`, { headers: authHeaders() });
  ...
  const approveStudent = async (userId: string) => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/admin/approve/${userId}`, { method: "POST", headers: authHeaders() });
  ...
  const removeStudent = async (userId: string) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE_URL}/admin/reject/${userId}`, { method: "POST", headers: authHeaders() });
      if (response.ok) fetchOrgUsers();
  ...
  return (
    <DashboardLayout
      ...
      title={TAB_TITLES[activeTab] ?? "Admin Dashboard"}
      navItems={navItems}

                  ...
                  {screen.questions.map((q) => (
                    <div key={q.id} className={`sv-field${showErrors && !isAnswered(q, answers[q.id]) ? " sv-field-error" : ""}`}>
                      <label className="sv-field-label">
                        {screen.kind === "single" || section?.id === "reflect" ? <span className="sv-field-question">{q.label}{q.required && <span className="sv-req"> *</span>}</span> : <>{q.label}{q.required && <span className="sv-req"> *</span>}</>}
                      </label>

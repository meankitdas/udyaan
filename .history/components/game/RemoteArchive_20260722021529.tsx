...
export function RemoteArchive() {
  ...
  const visibleFolders = useMemo(
    () => FOLDERS.filter((folder) => folder.name.toLowerCase().includes(searchQuery.toLowerCase())).sort((a, b) => sortAscending ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name)),
    [searchQuery, sortAscending],
  );
  const visibleDocs = useMemo(
    () => (currentFolder ? DOCS.filter((doc) => doc.folder === currentFolder) : [])
      .filter((doc) => `${doc.name} ${doc.type} ${doc.owner} ${doc.reference}`.toLowerCase().includes(searchQuery.toLowerCase()))
  ...
  const visibleLocalItems = useMemo(
    () => LOCAL_ITEMS[localLocation]
      .filter((item) => `${item.name} ${item.type}`.toLowerCase().includes(searchQuery.toLowerCase()))
      .sort((a, b) => sortAscending ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name)),
  ...
  return (
    <main className={styles.shell}>
      ...
      {stage === "rdp" && localPreview && (
        <section className={styles.documentWindow} aria-label={localPreview.name}>
          <div className={styles.titlebar}>
            <span><LocalGlyph item={localPreview} size={17} /> {localPreview.name}</span>
            <WindowButtons onMinimize={() => setLocalPreview(null)} onMaximize={() => setExplorerMaximized((value) => !value)} onClose={() => setLocalPreview(null)} />
          </div>
      ...
      {activePanel === "start" && (
        <section className={styles.startMenu}>
          ...
          <div className={styles.pinnedApps}>
            ...
            <button type="button" onClick={() => navigateToFolder("cohort")}><Users color="#5a70c7" /><span>Student Data</span></button>
            <button type="button" onClick={() => navigateToFolder("finance")}><FileSpreadsheet color="#24865a" /><span>Financials</span></button>
          </div>
      ...
      {activePanel === "search" && (
        <section className={styles.searchPanel}>
          <label><Search size={17} /><input autoFocus value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search Operation CANOPY" /></label>
          <p>Best match</p>
          <div className={styles.searchResults}>
            {FOLDERS.filter((folder) => folder.name.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 4).map((folder) => (
            ...
            {DOCS.filter((doc) => `${doc.name} ${doc.summary}`.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 6).map((doc) => (
      ...
      {activePanel === "taskview" && (
        <section className={styles.taskView}>
          ...
          <div className={styles.taskCards}>
            <button type="button" onClick={() => { setExplorerVisible(true); setActivePanel(null); }}><span className={styles.taskPreview}><Folder size={35} fill="#f3c94f" color="#dcae2f" /></span><b>Project Archive</b><small>File Explorer · JGI-ARCHIVE-02</small></button>
            <button type="button" onClick={() => setActivePanel("remote")}><span className={styles.taskPreview}><Monitor size={35} color="#3b7fca" /></span><b>Remote Desktop</b><small>Connected · campus\temporary-user</small></button>
          </div>
        </section>
      )}
      ...
      {(activePanel === "settings" || activePanel === "remote") && (
        <section className={styles.systemPanel}>
          ...
          {activePanel === "settings" ? <><h2>System</h2><p><b>Display</b><small>1280 × 800 · Remote display adapter</small></p><p><b>Network & internet</b><small>{wifiOn ? "Connected through campus guest gateway" : "Wi-Fi disabled"}</small></p><p><b>Privacy & security</b><small>Session monitoring active</small></p></> : <><h2>Remote session</h2><p><b>Computer</b><small>{server}</small></p><p><b>User</b><small>{user}</small></p><p><b>Connection quality</b><small>Excellent · UDP enabled</small></p><button type="button" className={styles.disconnectButton} onClick={resetConnection}><LogOut size={16} /> Disconnect</button></>}
        </section>
      )}
      ...
      {stage !== "connecting" && <Taskbar
        clock={clock}
        active={stage === "rdp" && rdpVisible ? "remote" : explorerVisible && stage !== "rdp" ? "explorer" : null}

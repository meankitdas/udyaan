      ...
      {activePanel === "search" && (
        <section className={styles.searchPanel}>
          ...
          <div className={styles.searchResults}>
            {!remoteConnected && Object.values(LOCAL_ITEMS).flat().filter((item) => `${item.name} ${item.type}`.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 8).map((item) => (
            ...
            {remoteConnected && FOLDERS.filter((folder) => folder.name.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 4).map((folder) => (
            ...
            {remoteConnected && DOCS.filter((doc) => `${doc.name} ${doc.summary}`.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 6).map((doc) => (

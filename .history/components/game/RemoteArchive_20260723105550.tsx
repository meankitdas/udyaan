...
function WindowButtons({
  ...
  if (disabled) {
    return (
      <div className={`${styles.windowButtons} ${styles.windowButtonsDisabled}`} aria-hidden>
...
export function RemoteArchive() {
  ...
  useEffect(() => {
    ...
    const timers = [
      ...
      window.setTimeout(() => { setStage("desktop"); setCurrentFolder("cohort"); }, 2550),
    ];
  ...
  useEffect(() => {
    ...
    // Trigger quickly once the visitor has opened a couple of files; otherwise a
    // gentle fallback timer keeps the story moving for anyone who just looks around.
    const delay = opened.size >= 2 ? 800 : 22000;
  ...
  useEffect(() => {
    ...
    setRecoveryStep(0);
    const timers = [
      window.setTimeout(() => setRecoveryStep(1), 700),
      window.setTimeout(() => setRecoveryStep(2), 1400),
      window.setTimeout(() => setStage("reveal"), 2500),
    ];
    return () => timers.forEach(window.clearTimeout);
  }, [stage]);
    ...
    <main className={styles.shell}>
      ...
      {stage === "rdp" && rdpVisible && (
        <section className={`${styles.rdpWindow} ${rdpMaximized ? styles.rdpMaximized : ""}`} aria-label="Remote Desktop Connection">
          ...
          <div className={styles.rdpFooter}>
            <span className={styles.connectionRequired}>Click Connect to open the leaked drive</span>
            <button type="button" className={`${styles.winButton} ${styles.winButtonPulse}`} onClick={connect}>Connect</button>
          </div>
        </section>
      )}
      ...
      {stage === "connecting" && (
        <section className={styles.connecting}>
          ...
          <p className={styles.connectingRequired}>Please wait. This connection cannot be cancelled.</p>
        </section>
      )}
      ...
      {stage !== "rdp" && stage !== "connecting" && (
        <>
          ...
          {stage === "desktop" && !triggered && (
            <div className={styles.objective}>
              <span className={styles.objectiveDot} />
              Open any 2 files to expose the secret
              <b>{Math.min(opened.size, 2)}/2</b>
            </div>
          )}
          ...
          {(stage === "locked" || stage === "recovery") && (
            <div className={styles.securityOverlay}>
              {stage === "locked" ? (
                <section className={styles.securityDialog}>
                  ...
                  <div className={styles.securityContent}>
                    ...
                    <div>
                      ...
                      <p className={styles.securityNote}>Your session is locked. Tap <b>Regain access</b> to restore it and continue.</p>
                    </div>
                  </div>
                ...
                <section className={styles.recoveryWindow}>
                  <div className={styles.recoveryTitle}><span><LockKeyhole size={17} /> Restoring access</span><span>JGI-SOC</span></div>
                  <div className={styles.terminal}>
                    <p>re-authenticating session ...... <b className={styles.ok}>ok</b></p>
                    {recoveryStep >= 1 && <p>rebuilding secure channel ...... <b className={styles.ok}>ok</b></p>}
                    {recoveryStep >= 2 && <p>decrypting programme identity ... <b className={styles.ok}>ok</b></p>}
                    <div className={styles.recoveryProgress}><i style={{ width: `${(recoveryStep + 1) * 33}%` }} /></div>
                    <p className={styles.accessRestored}>{recoveryStep >= 2 ? "ACCESS RESTORED" : "Please wait\u2026"}</p>
                  </div>
                </section>
              )}
            </div>
      ...
      {remoteConnected && <Taskbar
        clock={clock}
        active={activePanel === "remote" ? "remote" : explorerVisible ? "explorer" : null}

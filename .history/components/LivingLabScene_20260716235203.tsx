...
export function LivingLabScene() {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0x102d21, 32, 72);

    const camera = new THREE.PerspectiveCamera(34, 1, 0.1, 120);
    camera.position.set(10.4, 6.8, 12.6);
    ...
    const markerAnchors = [
      new THREE.Vector3(-2.4, 3.15, -1.7),
      new THREE.Vector3(1.15, 4.85, -0.5),
      new THREE.Vector3(4.5, 2.95, -1.65),
      new THREE.Vector3(5.25, 2.15, 3.25),
    ].map((position) => {
    ...
    const framingBox = new THREE.Box3().setFromObject(campus);
    const framingSphere = framingBox.getBoundingSphere(new THREE.Sphere());
    const framingCenter = framingSphere.center.clone();
    const framingRadius = framingSphere.radius;
    const viewDirection = new THREE.Vector3(0.62, 0.46, 0.78).normalize();

    const frameCamera = (aspect: number) => {
      const vFov = THREE.MathUtils.degToRad(camera.fov);
      const fitHeightDistance = framingRadius / Math.sin(vFov / 2);
      const fitWidthDistance = fitHeightDistance / Math.min(aspect, 1);
      const distance = Math.max(fitHeightDistance, fitWidthDistance) * 1.02;
    ...
    const updateMarkers = () => {
      ...
      markerAnchors.forEach((anchor, index) => {
        ...
        if (!element) return;
        anchor.getWorldPosition(anchorWorldPosition).project(camera);
        const x = (anchorWorldPosition.x * 0.5 + 0.5) * bounds.width;
        const y = (-anchorWorldPosition.y * 0.5 + 0.5) * bounds.height;
        element.style.transform = `translate(${x}px, ${y}px) translate(-50%, calc(-100% - 44px))`;
      });
    };
    ...
    const resize = () => {
      ...
      console.log("LABDEBUG", JSON.stringify({ bw: bounds.width, bh: bounds.height, aspect, center: framingCenter.toArray(), radius: framingRadius, camPos: camera.position.toArray(), fov: camera.fov }));
    };
  ...
  return (
    <div ref={mountRef} className={`living-lab-scene${ready ? " is-ready" : ""}`}>
      ...
      {["Controlled environment", "Drone intelligence", "Hydroponic rack", "Bio-CNG loop"].map((label, index) => (
        <span
          key={label}

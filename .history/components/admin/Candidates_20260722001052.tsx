...
import { evaluateResponse, fetchEngineStatus, listResponses } from "@/lib/api";
...
export function Candidates({ form }: { form: SurveyForm }) {
  ...
  async function screenAll() {
    setBulkRunning(true);
    for (const r of responses.filter((r) => !r.evaluation)) {
      await screen(r);
    }
      ...
      <div className="cd-toolbar">
        ...
        <div className="cd-toolbar-right">
          ...
          <motion.button
            ...
            onClick={screenAll}
            disabled={bulkRunning || counts.unscreened === 0}
            whileTap={{ scale: 0.97 }}
          >
            {bulkRunning ? "Screening\u2026" : `Screen ${counts.unscreened} unscreened`}
          </motion.button>
        </div>
      </div>

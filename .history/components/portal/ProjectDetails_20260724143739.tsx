...
import { API_BASE_URL, authHeaders, friendlyError, getToken, roleHome } from "@/lib/portal-api";
...
export default function ProjectDetails() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const profileRes = await fetch(`${API_BASE_URL}/auth/me`, { headers: authHeaders() });
      if (profileRes.ok) {
      ...
      const projectRes = await fetch(`${API_BASE_URL}/projects/${id}`, { headers: authHeaders() });
      if (projectRes.ok) {
      ...
      const meetingsRes = await fetch(`${API_BASE_URL}/projects/${id}/meetings`, { headers: authHeaders() });
      if (meetingsRes.ok) setMeetings(await meetingsRes.json());

      const actionsRes = await fetch(`${API_BASE_URL}/projects/${id}/action-items`, { headers: authHeaders() });

...
export default function StudentDashboard() {
  ...
  const fetchProjects = async () => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/projects`, { headers: authHeaders() });
  ...
  const fetchActions = async () => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/action-items/me`, { headers: authHeaders() });
                  ...
                  <tbody>
                    {actions.map((a) => (
                      <tr key={a.id}>
                        ...
                        <td style={{ textAlign: "right" }}>
                          <Link
                            href={`/portal/projects/${a.project_id}`}
                            className="btn-link"
                            style={{ display: "inline-flex", alignItems: "center", gap: "6px", textDecoration: "none" }}
                          >
                            <Eye size={15} strokeWidth={1.8} aria-hidden /> View Project
                          </Link>
                        </td>
                      </tr>
                    ))}
  ...
  return (
    <DashboardLayout
      ...
      title={TAB_TITLES[activeTab] ?? "Student Dashboard"}
      navItems={navItems}

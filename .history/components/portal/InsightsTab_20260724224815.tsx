...
export default function InsightsTab() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_BASE_URL}/community/insights`, { headers: authHeaders() }).then((r) => (r.ok ? r.json() : null)),
      fetch(`${API_BASE_URL}/community/leaderboard`, { headers: authHeaders() }).then((r) => (r.ok ? r.json() : [])),
    ])
      .then(([ins, lb]) => {
        setInsights(ins);
        setLeaderboard(lb);
    ...
    <div style={{ display: "grid", gap: "24px" }}>
      ...
      <div className="table-card">
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
          <h4 style={{ margin: 0, color: "var(--dark-green)" }}>✅ Org Task Completion</h4>
      ...
      <div className="grid-2-cols">
        {/* Upcoming deadlines */}
        <div className="table-card">
          <h4 style={{ margin: "0 0 16px 0", color: "var(--dark-green)" }}>⏰ Deadlines (Next 14 Days)</h4>
        ...
        {/* Leaderboard */}
        <div className="table-card">
          <h4 style={{ margin: "0 0 16px 0", color: "var(--dark-green)" }}>🏆 Top Contributors</h4>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {leaderboard.slice(0, 5).map((entry, i) => (
                <div key={entry.user_id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "8px 12px", borderRadius: "10px", backgroundColor: "#f9fafb", border: "1px solid #eee" }}>
                  <span style={{ fontSize: "1.2rem", width: "28px" }}>{i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i + 1}.`}</span>
                  <div style={{ flex: 1 }}>

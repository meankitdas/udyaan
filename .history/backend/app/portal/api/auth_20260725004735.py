...
async def refresh_token(data: RefreshTokenRequest, db: AsyncSession = Depends(get_db)):
    # 1. Verify the refresh token's signature, expiry and type before touching the DB.
    from app.portal.core.deps import decode_token

    decode_token(data.refresh_token, "refresh")
...
# Single shared implementation (validates token type, expiry and account status).

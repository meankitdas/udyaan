...
KNOWLEDGE_DOCS: list[dict] = [
    ...
    },
    {
        ...
        "text": (
            "Timing is supporting evidence worth no more than 5 points, not a proxy for ability. Every question "
            "has a minimum plausible read time estimated from its text length (about 5 words per second, plus "
            "extra for images). An answer submitted well below that floor means the student could not have read "
            "the question. If half or more of the answers fall below the read floor, or many fast answers arrive "
            "at near-identical intervals, treat it as a rapid-fill pattern: cap timing points at zero and route "
            "the candidate to human review rather than rejecting them. Isolated fast answers are normal. Slow "
            "readers must never be penalised; dwell up to two minutes per short choice question is credible. One "
            "or more answer changes can show healthy reconsideration. Never infer cheating, disability, language "
            "proficiency, or intelligence from timing alone."
        ),
    },
    {

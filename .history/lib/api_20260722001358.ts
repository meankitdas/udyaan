...
function heuristicEvaluation(form: SurveyForm, response: SurveyResponse): Evaluation {
  ...
  const criteria = {
    ...
    practical_problem_solving: Math.round(sectionAccuracy("level2") * 14 + (resourcePoints[answer("reflect_resource")] ?? 4)),
    strategic_decision_making: Math.round(sectionAccuracy("level3") * 10 + (answer("l3_q3") ? 5 : 0)),
    learning_mindset: Math.round((answer("reflect_interest") ? 4 : 0) + (answer("reflect_decision") ? 5 : 0) + (improvePoints[answer("reflect_improve")] ?? 3)),
    initiative_and_program_fit: Math.round((answer("reflect_interest") ? 4 : 0) + (ideaPoints[answer("reflect_idea")] ?? 3)),

...
def _min_read_seconds(question: Question) -> float:
    """Minimum plausible seconds to read a question and pick an answer.

    Uses a fast-skim reading rate (~5 words/second) over the label plus
    options, a small decision overhead, and extra time when an image must
    be looked at. Deliberately conservative so slow readers are never hurt
    and only physically-impossible speeds are flagged.
    """
    words = len(question.label.split())
    for option in question.options or []:
        words += len(option.split())
    seconds = words / 5.0 + 0.8
    if question.image:
        seconds += 1.2
    return round(max(1.5, seconds), 1)


def read_analysis(form: SurveyForm, response: SurveyResponse) -> dict:
    """Per-question check: did the student have time to actually read it?"""
    questions = _question_index(form)
    timed = [t for t in response.timings if t.question_id in questions and response.answers.get(t.question_id)]
    if not timed:
        return {
            "questions_timed": 0,
            "unread_count": 0,
            ...
            "uniform_pace": False,
            "rapid_fill": False,
            "careful_reader": False,
        }

    unread_ids: list[str] = []
    for t in timed:
        minimum = _min_read_seconds(questions[t.question_id])
        if t.active_ms / 1000 < minimum * 0.6:  # 40% grace below the skim floor
            unread_ids.append(t.question_id)

    dwells = [t.active_ms / 1000 for t in timed]
    mean = sum(dwells) / len(dwells)
    variance = sum((d - mean) ** 2 for d in dwells) / len(dwells)
    spread = variance ** 0.5
    # Human reading varies with question length; near-identical fast dwells
    # across many questions look scripted or click-through.
    uniform_pace = len(dwells) >= 5 and mean < 6 and spread < 1.0

    unread_rate = len(unread_ids) / len(timed)
    return {
        "questions_timed": len(timed),
        "unread_count": len(unread_ids),
        "unread_rate": round(unread_rate, 3),
        "read_rate": round(1 - unread_rate, 3),
        "unread_question_ids": unread_ids[:12],
        "uniform_pace": uniform_pace,
        "rapid_fill": unread_rate >= 0.5 or (uniform_pace and unread_rate >= 0.3),
        "careful_reader": unread_rate <= 0.15 and not uniform_pace,
    }
...
def timing_stats(form: SurveyForm, response: SurveyResponse) -> dict:
    ...
    reading = read_analysis(form, response)
...
def assessment_parameters(
    ...
    if not response.timings:
        timing = 3.0
    elif stats["rapid_fill"]:
        timing = 0.0
    elif stats["rushed_quiz"] or stats["unread_rate"] >= 0.3:
        timing = 1.0
    elif stats["careful_reader"] and stats["deliberate_pace"]:
        timing = 5.0
...
def calibrate_cohort(responses: list[SurveyResponse]) -> list[SurveyResponse]:
    """Rank evaluated candidates and target a 15% shortlist within a 10-20% policy."""
    ranked = sorted(
        (response for response in responses if response.evaluation is not None),
        key=lambda response: (
            response.evaluation.score,
            response.evaluation.criteria.get("farm_logic_accuracy", 0),
            response.evaluation.criteria.get("practical_problem_solving", 0),
            response.evaluation.criteria.get("strategic_decision_making", 0),
    ...
    minimum = max(1, (cohort_size + 9) // 10)
    maximum = max(minimum, cohort_size // 5)
    target = min(maximum, max(minimum, round(cohort_size * 0.15)))
    eligible = [
        response
        for response in ranked
        if response.evaluation.score >= REVIEW_SCORE
        and response.evaluation.criteria.get("timing_credibility", 3) > 1
    ]
    shortlist_ids = {response.id for response in eligible[:target]}

    for rank, response in enumerate(ranked, start=1):
        evaluation = response.evaluation
        evaluation.cohort_rank = rank
        evaluation.cohort_size = cohort_size
        evaluation.cohort_percentile = round(rank / cohort_size * 100, 1)
        if response.id in shortlist_ids:
            evaluation.verdict = "shortlist"
        elif evaluation.score >= REVIEW_SCORE:
...
def _answers_digest(form: SurveyForm, response: SurveyResponse) -> str:
    ...
    for section in form.sections:
        for q in section.questions:
            ...
            suffix = ""
            if t:
                dwell_s = t.active_ms / 1000
                floor_s = _min_read_seconds(q)
                suffix = f" [dwell {dwell_s:.0f}s, min-read {floor_s:.0f}s, {t.changes} changes]"
                if dwell_s < floor_s * 0.6:
                    suffix += " [FASTER THAN POSSIBLE READ]"
...
def evaluate_response(form: SurveyForm, response: SurveyResponse) -> Evaluation:
    ...
    query = (
        "screening rubric farm logic practical problem solving strategic decisions learning mindset "
        f"initiative engagement timing fairness cohort selection {reflect_text[:600]}"
    )
...
def _azure_evaluate(
    ...
    if (stats["rushed_quiz"] or stats["rapid_fill"]) and verdict == "shortlist":
        verdict = "review"
...
def _heuristic_evaluate(
    ...
    if stats["careful_reader"]:
        strengths.append(f"Read {stats['read_rate'] * 100:.0f}% of questions at a humanly plausible pace.")
    elif stats["deliberate_pace"]:
        strengths.append("Pacing supports considered responses.")
    if stats["rapid_fill"]:
        concerns.append(
            f"Rapid-fill pattern: {stats['unread_count']}/{stats['questions_timed']} questions answered faster "
            f"than the minimum read time{' with near-uniform pacing' if stats['uniform_pace'] else ''}."
        )
    elif stats["unread_rate"] >= 0.3:
        concerns.append(
            f"{stats['unread_count']}/{stats['questions_timed']} answers were faster than the question could be read."
        )
    ...
    return Evaluation(
        ...
        timingAnalysis=(
            ...
            f"Read-time check: {stats['unread_count']}/{stats['questions_timed']} answers landed below the "
            f"per-question minimum read time. "
            + (
                "Rapid-fill pattern detected \u2014 answers arrived faster than the questions could be read."
                if stats["rapid_fill"]
                else "Rush pattern detected."
                if stats["rushed_quiz"]
                else "Pacing consistent with genuinely reading each question."
                if stats["careful_reader"]
